...
class Environment {
 public:
  /**
   * Add a client to an environment, and process and save their image
   * descriptors and keypoints. If another client in the environment exists, run
   * SIFT and compute matches given that other image.
   */
  std::shared_ptr<HomographyTransformResult> updateClient(
      EntityID id,
      std::string image,
   ...
   * but also use the previous homographies and update the anchor points for
   * any clients that don't have 2D anchor point yet.
   *
   ...
   * transformed point from the original homography to find an anchor point
